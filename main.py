...
def update_nvr(ip: str, p: dict, session: Session = Depends(get_session), user: dict = Depends(require_admin)):
    ...
    if new_ip and new_ip != ip:
        ...
        log_event(session, category="NVR", action="NVR_UPDATE", details=f"تنظیمات NVR ({new_nvr.name or new_ip}) و آدرس IP آن از {ip} به {new_ip} بروزرسانی گردید", level="INFO", actor_username=user.get("username","admin"), group_id=new_nvr.group_id, target_type="NVR", target_id=new_ip)
        return new_nvr
