...
from app.services.monitor import set_broadcast_callback, start_monitor_loop
...
def seed_database(session: Session):
    defaults = {
        ...
        "OUTAGE_MIN_HOURS_TO_EXPLAIN": (
            "2",
            "حداقل زمان قطعی به ساعت برای نیاز به رفع ابهام",
        ),
        "OUTAGE_EXPLANATION_DEADLINE_HOURS": ("24", "مهلت رفع ابهام قطعی به ساعت"),
        "OUTAGE_ANALYSIS_DAYS": (
            "5,6,0,1,2,3",
            "روزهای بررسی قطعی در هفته (شنبه=5 تا جمعه=4)",
        ),
    ...
    with Session(engine) as session:
        ...
        if existing_settings_count > 0:
            ...
            defaults = {
                ...
                "OUTAGE_EXPLANATION_DEADLINE_HOURS": (
                    "24",
                    "مهلت رفع ابهام قطعی به ساعت",
                ),
                "OUTAGE_ANALYSIS_DAYS": (
                    "5,6,0,1,2,3",
                    "روزهای بررسی قطعی در هفته (شنبه=5 تا جمعه=4)",
            ...
            for key, (default_val, desc) in defaults.items():
                if not session.get(Settings, key):
                    session.add(
                        Settings(key=key, value=str(default_val), description=desc)
                    )
            session.commit()
            return
        ...
        defaults = {
            ...
            "LIMIT_LOGIN_MAX_ATTEMPTS": (
                "5",
                "حداکثر تلاش‌های ورود ناموفق مجاز در دقیقه از یک IP",
            ),
            "LIMIT_PING_TIMEOUT_SECONDS": ("2", "تایم‌اوت پینگ اتصال دوربین‌ها (ثانیه)"),
            "LIMIT_SNAPSHOT_TIMEOUT_SECONDS": (
                "5",
                "تایم‌اوت دریافت تصویر پیش‌نمایش به ثانیه",
            ),
            "LIMIT_API_RATE_LIMIT_PER_MIN": (
                "60",
                "سقف درخواست‌های مجاز عمومی API در دقیقه",
            ),
            "LIMIT_LOG_RETENTION_DAYS": (
                "90",
                "مدت زمان نگه‌داری لاگ‌های قطعی و مانیتورینگ (روز)",
            ),
        }
        defaults.update(notification_default_settings())
...
def verify_admin_password(
    input_password: str, env_password_val: str
) -> tuple[bool, bool]:
    ...
    if user["role"] == "admin":
        ...
        if db_user.accessible_group_ids:
            ...
            try:
                ids = [
                    int(x.strip())
                    for x in db_user.accessible_group_ids.split(",")
                    if x.strip().isdigit()
                ]
                if ids:
                    return ids
...
def login(
    ...
    if not check_rate_limit(client_ip):
        raise HTTPException(
            status_code=429,
            detail="تعداد تلاش‌ها بیش از حد مجاز است. لطفاً یک دقیقه صبر کنید",
        )
    ...
    if payload.username == admin_user and password_ok:
        ...
        if password_is_plain:
            logger.warning(
                "SECURITY WARNING: Admin password in .env is stored in plain text (not hashed). Please hash it and replace. Guide: static/admin-password-help.html"
            )
        ...
        log_event(
            ...
            details=f"ورود موفق کاربر مدیر ({payload.username})",
            level="INFO",
    ...
    if db_user and verify_password(payload.password, db_user.password_hash):
        ...
        log_event(
            ...
            category="Security",
            action="LOGIN_SUCCESS",
            details=f"ورود موفق کاربر ({db_user.username})",
            level="INFO",
    ...
    log_event(
        ...
        category="Security",
        action="LOGIN_FAILED",
        details=f"تلاش ناموفق برای ورود با نام کاربری ({payload.username})",
        level="WARNING",
        actor_username=payload.username,
        actor_ip=client_ip,
    )
...
def login_2fa(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="توکن موقت نامعتبر یا منقضی شده است"
        )
    ...
    if not expires_at_str or datetime.fromisoformat(expires_at_str) < datetime.now():
        raise HTTPException(
            status_code=400,
            detail="زمان ورود کد به پایان رسیده است. لطفاً مجدداً تلاش کنید",
        )
    ...
    if not totp.verify(payload.code):
        raise HTTPException(
            status_code=400, detail="کد وارد شده صحیح نیست یا منقضی شده است"
        )
    ...
    if token:
        ...
        if session_record:
            log_event(
                ...
                category="Security",
                action="LOGOUT",
                details=f"خروج کاربر ({session_record.username})",
                level="INFO",
                actor_username=session_record.username,
...
app.mount(
    "/static/snapshots", StaticFiles(directory="data/snapshots"), name="snapshots"
)
...
def update_task_interval(
    task_id: str,
    payload: UpdateIntervalRequest,
...
async def purge_database(session: Session = Depends(get_session)):
    ...
    log_event(
        ...
        details="دیتابیس سیستم به تنظیمات اولیه ریست گردید",
        level="CRITICAL",
...
def backup_database(session: Session = Depends(get_session)):
    ...
    log_event(
        ...
        details="فایل پشتیبان کامل دیتابیس دانلود شد",
        level="INFO",
        actor_username="admin",
    )
...
async def restore_database(request: Request, file: UploadFile = File(...)):
    ...
    while True:
        ...
        if size > MAX_SIZE:
            raise HTTPException(
                status_code=413,
                detail="حجم فایل پشتیبان بیش از حد مجاز (۵۰ مگابایت) است",
            )
        chunks.append(chunk)
    ...
    with Session(engine) as session:
        log_event(
            ...
            details="فایل پشتیبان دیتابیس بازیابی (Restore) گردید",
            level="CRITICAL",
            actor_username="admin",
        )
    return {"status": "ok"}
...
def export_config_json(session: Session = Depends(get_session)):
    log_event(
        ...
        details="پیکربندی سیستم به صورت فایل JSON دانلود گردید",
        level="INFO",
        actor_username="admin",
    )
    # 1. Settings
    ...
    all_alert_settings = (
        session.exec(
            select(UserAlertSettings).where(UserAlertSettings.user_id.in_(user_ids))
        ).all()
        if user_ids
        else []
    )
...
async def import_config_json(request: Request, session: Session = Depends(get_session)):
    ...
    log_event(
        ...
        details="پیکربندی جدید از فایل JSON وارد (Import) گردید",
        level="CRITICAL",
        actor_username="admin",
    )
    return {"status": "ok"}
...
def test_mail(request: Request):
    ...
    res = send_email_raw(
        conf,
        "تست سامانه مانیتورینگ",
        "<div style='text-align:center;padding:20px;'><h3 style='color:#28a745;'>ایمیل به درستی کار میکنه!</h3><p>تاریخ: "
        + get_persian_datetime()
        + "</p></div>",
        recipients,
    )
...
def test_telegram(request: Request):
    ...
    res = send_telegram_raw(
        conf,
        "✅ <b>تست سامانه مانیتورینگ</b>\nاعلان‌های تلگرام درسته!\n📅 "
        + get_persian_datetime(),
        chat_ids,
    )
    if res is True:
...
def get_groups(
    session: Session = Depends(get_session), user: dict = Depends(require_auth)
):
    ...
    if not accessible_groups:
        return []
    return session.exec(
        select(NVRGroup).where(NVRGroup.id.in_(accessible_groups))
    ).all()
...
def create_group(
    ...
    log_event(
        ...
        category="Config",
        action="GROUP_CREATE",
        details=f"گروه/کارخانه جدید ({group.name}) ایجاد شد",
        level="INFO",
...
def update_group(
    ...
    if "map_center_lat" in p:
        g.map_center_lat = (
            float(p["map_center_lat"]) if p["map_center_lat"] is not None else None
        )
    if "map_center_lng" in p:
        g.map_center_lng = (
            float(p["map_center_lng"]) if p["map_center_lng"] is not None else None
        )
    if "map_zoom" in p:
    ...
    log_event(
        ...
        category="Config",
        action="GROUP_UPDATE",
        details=f"اطلاعات گروه/کارخانه ({g.name}) ویرایش شد",
        level="INFO",
        actor_username=user.get("username", "admin"),
        group_id=g.id,
        target_type="Group",
...
def delete_group(
    ...
    log_event(
        ...
        category="Config",
        action="GROUP_DELETE",
        details=f"گروه/کارخانه ({g_name}) حذف شد",
        level="WARNING",
        actor_username=user.get("username", "admin"),
        target_type="Group",
        target_id=id,
...
def get_group_plans(
    id: int, session: Session = Depends(get_session), user: dict = Depends(require_auth)
):
    g = session.get(NVRGroup, id)
    if not g:
        raise HTTPException(status_code=404, detail="Group not found")
    plans = session.exec(
        select(MapPlan).where(MapPlan.group_id == id).order_by(MapPlan.sort_order)
    ).all()
    return [
        {
            "id": p.id,
...
async def upload_group_plan(
    ...
    if file.size and file.size > 5 * 1024 * 1024:
        raise HTTPException(
            status_code=413, detail="حجم فایل بیش از حد مجاز (۵ مگابایت) است"
        )
    ...
    if ext not in ["png", "jpg", "jpeg", "svg"]:
        raise HTTPException(
            status_code=400, detail="فرمت فایل باید JPG، PNG یا SVG باشد"
        )
    ...
    if user["role"] == "admin":
        events = session.exec(
            select(DowntimeEvent).where(
                ...
                | (DowntimeEvent.end_time >= thirty_days_ago)
            )
        ).all()
    else:
...
def search_logs(
    ...
    if user["role"] != "admin":
        ...
        if accessible_groups is not None:
            query = query.where(
                (col(Log.group_id).in_(accessible_groups))
                | (col(Log.actor_username) == user["username"])
            )
        elif user.get("group_id") is not None:
            query = query.where(
                (col(Log.group_id) == user["group_id"])
                | (col(Log.actor_username) == user["username"])
            )
        else:
...
def generate_report(
    ...
    task_logs = session.exec(
        select(Log)
        .where(
            Log.log_type == "Task", Log.timestamp >= start_dt, Log.timestamp <= end_dt
        )
        .order_by(Log.timestamp.desc())
    ).all()
...
def get_reports_charts(
    ...
    if accessible_groups is not None:
        nvrs_objs = session.exec(
            select(NVR).where(NVR.group_id.in_(accessible_groups))
        ).all()
    ...
    explanations_query = select(OutageExplanation).where(
        OutageExplanation.start_time >= start_dt, OutageExplanation.start_time <= end_dt
    )
    if accessible_groups is not None:
        explanations_query = explanations_query.where(
            OutageExplanation.group_id.in_(accessible_groups)
        )
    ...
    for o in explanations:
        if o.explanation_type:
            causes_count[o.explanation_type] = (
                causes_count.get(o.explanation_type, 0) + 1
            )
...
class UserCreate(BaseModel):
    ...
    accessible_group_ids: str | None = None
...
def validate_accessible_groups(accessible_group_ids: str | None, session: Session):
    ...
    for p in parts:
        if not p.isdigit():
            raise HTTPException(
                status_code=400, detail=f"شناسه گروه '{p}' باید عدد باشد"
            )
        g_id = int(p)
        if not session.get(NVRGroup, g_id):
            raise HTTPException(
                status_code=400, detail=f"گروه با شناسه {g_id} وجود ندارد"
            )


@app.get("/api/users", response_model=list[User])
def get_users(
    session: Session = Depends(get_session), user: dict = Depends(require_admin)
):
...
def create_user(
    ...
    if payload.group_id is not None:
        if not session.get(NVRGroup, payload.group_id):
            raise HTTPException(
                status_code=400, detail=f"گروه با شناسه {payload.group_id} وجود ندارد"
            )
    ...
    log_event(
        ...
        category="User",
        action="USER_CREATE",
        details=f"کاربر جدید ({payload.username}) با نقش '{payload.role}' ایجاد شد",
        level="INFO",
        actor_username=user.get("username", "admin"),
        group_id=payload.group_id,
        target_type="User",
...
def update_user(
    ...
    if db_user.id == user["user_id"] or db_user.username == user["username"]:
        if payload.role is not None and payload.role != db_user.role:
            raise HTTPException(
                status_code=400, detail="شما نمی‌توانید نقش کاربری خود را تغییر دهید"
            )
        if payload.is_active is False:
            raise HTTPException(
                status_code=400, detail="شما نمی‌توانید حساب کاربری خود را غیرفعال کنید"
            )
    ...
    if (
        payload.accessible_group_ids is not None
        or "accessible_group_ids" in payload.model_fields_set
    ):
        validate_accessible_groups(payload.accessible_group_ids, session)
    ...
    if (
        payload.accessible_group_ids is not None
        or "accessible_group_ids" in payload.model_fields_set
    ) and payload.accessible_group_ids != db_user.accessible_group_ids:
        changes.append(
            f"دسترسی‌های چندگانه از '{db_user.accessible_group_ids}' به '{payload.accessible_group_ids}'"
        )
    if payload.password:
...
def delete_user(
    ...
    alert_settings = session.exec(
        select(UserAlertSettings).where(UserAlertSettings.user_id == id)
    ).first()
    if alert_settings:
    ...
    log_event(
        ...
        category="User",
        action="USER_DELETE",
        details=f"کاربر ({u_name}) حذف گردید",
        level="WARNING",
        actor_username=user.get("username", "admin"),
        group_id=g_id,
        target_type="User",
...
def get_my_alerts(
    ...
    if u_id is None:
        raise HTTPException(
            status_code=400, detail="مدیر سیستم از تنظیمات عمومی استفاده می‌کند"
        )

    alert_settings = session.exec(
        select(UserAlertSettings).where(UserAlertSettings.user_id == u_id)
    ).first()
...
def update_my_alerts(
    ...
    if payload.telegram_chat_ids and "," in payload.telegram_chat_ids:
        raise HTTPException(
            status_code=400, detail="فقط وارد کردن یک شناسه تلگرام مجاز است"
        )
...
def get_outage_explanations(
    ...
    active_causes = [
        ...
        for c in session.exec(
            select(OutageCause).where(OutageCause.is_active == True)
        ).all()
    ]
    ...
    for o in outages:
        ...
        shamsi_explained_at = (
            format_shamsi_datetime(o.explained_at) if o.explained_at else None
        )
...
def submit_outage_explanation(
    id: int,
    payload: OutageExplanationSubmit,
    session: Session = Depends(get_session),
    user: dict = Depends(require_control),
):
    ...
    if user["role"] != "admin" and outage.group_id != user["group_id"]:
        raise HTTPException(
            status_code=403, detail="دسترسی غیرمجاز برای ویرایش قطعی این کارخانه"
        )
    ...
    cause = session.exec(
        select(OutageCause).where(
            OutageCause.name == payload.explanation_type, OutageCause.is_active == True
        )
    ).first()
    if not cause:
...
def submit_bulk_outage_explanations(
    payload: BulkOutageExplanationSubmit,
    session: Session = Depends(get_session),
    user: dict = Depends(require_control),
):
    ...
    for oid in payload.ids:
        ...
        if not is_admin and outage.group_id != user["group_id"]:
            raise HTTPException(
                status_code=403,
                detail=f"دسترسی غیرمجاز برای ویرایش قطعی کارخانه با شناسه {oid}",
            )
        ...
        if outage.explained_at and not is_admin:
            raise HTTPException(
                status_code=400,
                detail=f"ابهام قطعی با شناسه {oid} قبلاً رفع شده و برای کاربران غیر ادمین غیرقابل تغییر است",
            )

        if now > outage.assigned_deadline and not is_admin:
            raise HTTPException(
                status_code=400,
                detail=f"مهلت رفع ابهام قطعی با شناسه {oid} به پایان رسیده است",
            )
    ...
    return {
        "status": "ok",
        "message": f"رفع ابهام دسته‌جمعی برای {updated_count} قطعی با موفقیت انجام شد",
...
def create_outage_cause(
    payload: OutageCauseCreate,
    session: Session = Depends(get_session),
    user: dict = Depends(require_admin),
):
...
def delete_outage_cause(
    ...
    used = session.exec(
        select(OutageExplanation).where(
            OutageExplanation.explanation_type == cause.name
        )
    ).first()
    if used:
        ...
        return {
            "status": "disabled",
            "message": "این علت قبلاً در ثبت قطعی‌ها استفاده شده است؛ بنابراین غیرفعال گردید تا در انتخاب‌های جدید نمایش داده نشود.",
        }
    else:
...
def change_my_password_endpoint(
    ...
    if u_id is None:
        raise HTTPException(
            status_code=400,
            detail="رمز عبور مدیر سیستم باید از طریق تنظیمات سرور تغییر کند. <a href='/static/admin-password-help.html' target='_blank' style='color:#3b82f6;text-decoration:underline;font-weight:bold;margin-right:5px;'>[مشاهده راهنما]</a>",
        )
    ...
    active_sessions = session.exec(
        select(UserSession).where(UserSession.username == db_user.username)
    ).all()
