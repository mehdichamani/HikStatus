...
- [x] **حذف کد تکراری** بین `fetchDash` و `updateDashFromWS` — `app.js:1373-1574`
- **توضیح**: کدهای تکراری UI داشبورد حذف شده و `fetchDash` صرفا وظیفه فچ کردن دیتا و پاس دادن به `updateDashFromWS` را بر عهده گرفت.

- [x] **شکستن تابع `fetchDash`** به توابع کوچکتر — `app.js:63-266`
- **توضیح**: با استخراج آپدیت‌های UI به `updateDashFromWS`، تابع `fetchDash` به طور خودکار به یک تابع بسیار کوچکتر و تمیزتر تبدیل شد.
...
- [x] **صفحه نمایش تمام‌صفحه (kiosk mode)** برای مانیتورهای بزرگ
- **توضیح**: دکمه حالت کیوسک با HTML5 Fullscreen API، کلاس CSS اختصاصی `kiosk-mode` جهت مخفی‌سازی هدر و ناوبری، دکمه شناور خروج و مدیریت رویداد کلید ESC و تغییرات حالت مرورگر پیاده‌سازی و اصلاح شد.

- [x] **داشبورد ماژولار و شخصی‌سازی‌پذیر با قابلیت Drag & Drop**
- **توضیح**: بازطراحی داشبورد با امکان ورود به حالت ویرایش، جابجایی کارت‌ها با درگ و دراپ (HTML5 Drag & Drop API)، حذف و اضافه ویجت‌ها با دکمه `+` و مدال اختصاصی، بازنشانی چینش و ذخیره‌سازی وضعیت در `localStorage`.
